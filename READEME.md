...
### WordPressの場合（wp-env docker使用）
1. dev.config.jsonで`"wordpress": true,`に設定  
2. .wp-env.json：設定を確認
3. Dockerを立ち上げてターミナルでwp-envを起動
```
npm run wp-start
```
4. wp-setting.shを使用してWordPress初期設定を行う
```
...
5. `npm i`をターミナルへ入力
6. ダウンロードが始まります
7. WordPress反映用フォルダ`my-themes`が作成されるので`Wordpress`フォルダを`my-themes`内に移動しフォルダ名とstyle.css内記述をテーマ名に変更  
```
mv WordPress my-themes/任意の名前
```
8. `npm run dev`で開発ブラウザが起動します
- `npm run build`でファイルを書き出す  
